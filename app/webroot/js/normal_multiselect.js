if (!m_helper) var m_helper = {
    ...
    create: function(a) {
        ...
        if (a.id && (b.id = a.id), a.class && (b.className = a.class), a.attributes) for (var c in a.attributes) b.setAttribute(c, a.attributes[c]);
        if (a.data) for (var c in a.data) b.dataset[c] = a.data[c];
        return b;
    },
    div: function(a) {
    ...
    textField: function(a) {
        return a || (a = {}), a.tag = "input", a.attributes || (a.attributes = {}), a.attributes.type = "text", 
        this.create(a);
    },
    checkbox: function(a) {
        return a || (a = {}), a.tag = "input", a.attributes || (a.attributes = {}), a.attributes.type = "checkbox", 
        this.create(a);
    },
    each: function(a, b) {
    ...
    click: function(a) {
        if (a.fireEvent) el.fireEvent("onclick"); else {
...
Multiselect.prototype = {
    _createUI: function() {
        ...
        m_helper.insertAfter(a, this._item), a.appendChild(this._createInputField()), a.appendChild(this._createItemList()), 
    ...
    _createInputField: function() {
        ...
        return b.style.visibility = "hidden", b.innerHTML = 0, d.appendChild(a), d.appendChild(b), 
        d.appendChild(c), d;
    },
    _createItemList: function() {
        ...
        var c = this._createItem("span", -1, "Select all"), d = m_helper.div({
    ...
    _createItem: function(a, b, c) {
        ...
        return g.className = "multiselect-text", g.innerHTML = c, j.appendChild(f), j.appendChild(g), 
        h.appendChild(j), h;
    },
    ...
    _toggle: function(a, b) {
        ...
        a && (m_helper.each(document.getElementById(this._getIdentifier()).querySelectorAll(".multiselect-checkbox"), function(d) {
            d.dataset.val == a && (b && !d.checked ? (m_helper.check(d), c._onCheckBoxChange(d, c)) : !b && d.checked && (m_helper.uncheck(d), 
    ...
    _appendEvents: function() {
        ...
        var b = function() {
            ...
            if (!c || "" == c) m_helper.show(document.querySelector("#" + a._getItemListIdentifier() + " > span")), 
            m_helper.show(document.querySelector("#" + a._getItemListIdentifier() + " > hr")), 
            m_helper.showAll(document.querySelectorAll("#" + a._getItemListIdentifier() + " li")); else {
    ...
    _onCheckBoxChange: function(a, b) {
        a.dataset.multiselectElement ? (b._performSelectItem(a, b), b._updateSelectAll(b)) : b._performSelectAll(a, b), 
        b._forceUpdate();
    },
    _performSelectItem: function(a, b) {
        var c = JSON.parse(a.dataset.multiselectElement);
        a.checked ? (b._itemCounter++, m_helper.select(this._item.options[c.index]), m_helper.setActive(a.parentElement.parentElement)) : (b._itemCounter--, 
        m_helper.deselect(this._item.options[c.index]), m_helper.setUnactive(a.parentElement.parentElement));
    },
    _performSelectAll: function(a, b) {
        var c = b._getItems();
        a.checked ? (b._itemCounter = c.length, m_helper.each(c, function(d) {
            m_helper.setActive(d.multiselectElement.parentElement.parentElement), m_helper.select(b._item.options[d.index]), 
            m_helper.check(d.multiselectElement);
        })) : (b._itemCounter = 0, m_helper.each(c, function(d) {
            d.multiselectElement.parentElement.parentElement.classList.remove("active"), b._item.options[d.index].removeAttribute("selected"), 
    ...
    _hideList: function(a, b) {
        m_helper.setUnactive(document.getElementById(a._getItemListIdentifier())), m_helper.show(document.getElementById(a._getItemListIdentifier()).querySelector("span")), 
    ...
    _updateText: function(a) {
        ...
        b.length == document.getElementById(a._getItemListIdentifier()).querySelectorAll("ul li").length && (c = "All selected"), 
        document.getElementById(a._getInputFieldIdentifier()).value = c ? c : "";
    },
    _showList: function(a) {
        document.getElementsByClassName('multiselect-list')[0].style.visibility = 'visible'; 
    ...
    _forceUpdate: function() {
        ...
        if (a.style.visibility = "hidden", 0 != this._itemCounter) {
            ...
            10 > this._itemCounter ? (a.style.left = "-45px", b.style.marginLeft = "-42px") : 100 > this._itemCounter ? (a.style.left = "-50px", 
            b.style.marginLeft = "-47px") : 1e3 > this._itemCounter ? (a.style.left = "-55px", 
            b.style.marginLeft = "-52px") : 1e4 > this._itemCounter && (a.style.left = "-60px", 
            b.style.marginLeft = "-57px");
        }
    },
    _items: void 0,
    _itemCounter: 0,
    ...
    _getItemUniqueIdentifier: function() {
        var a = this._item.getAttribute("id"), b = this._item.getAttribute("name");
...
function hideMultiselects(a) {
    // var multi = document.getElementsByClassName("multiselect-list");   
    // if(multi){
    //     document.getElementsByClassName('multiselect-list')[0].style.visibility = 'hidden';
    // }
