...
//die;
    ...
    function saveOrder($saveCustomer=1)
    {
		...
		$subject = $this->emailCustomerBooking($this->data['Order']['id']);
		...
		if(isset($_REQUEST['havetoprint']) && $_REQUEST['havetoprint']==1){
			...
			$db =& ConnectionManager::getDataSource($this->User->useDbConfig);
			//echo "UPDATE ace_rp_orders set emal_bounce_status='".$emal_bounce_status."' WHERE id = '".$order_id."'";die;
...
//echo $this->data['Order']['order_status_id'];
//echo "22".$_POST['havetoprint'];
...
echo "1";die;
	...
	function sendEmailUsingMailgun($to,$subject,$body,$order_id){
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL,"http://www.acecare.ca/acesystem2018/mailcheck.php");
	...
	function manageMailgunEmailLogs($msgid, $subject, $order_id){
		...
		return $result = $db->_execute($queryUpdate);
	}
...
//	      $past_orders = $this->Order->findAll(array('Order.customer_phone'=> $phone), null, "job_date DESC", null, null, 1);
  ...
  function _itemHTML($index, $item, $actions)
  {
	...
	elseif ($item['item_id']==$this->member_card1_item_id || $item['item_id']==$this->member_card2_item_id) { // Added by Maxim Kudryavtsev - member card booking
		...
		$h .= 'Member Card '.( $item['item_id']==$this->member_card1_item_id ? 1:2 ).'y #<input type="text" id="data[Order][BookingItem]['.$index.'][name]" name="data[Order][BookingItem]['.$index.'][name]" value="'.$item['name'].'" onfocus="if (value==\'enter number\') value=\'\';"/><br />';
		$h .= 'Next Service Date <input type="text" id="data[Order][BookingItem]['.$index.'][next_service]" name="data[Order][BookingItem]['.$index.'][next_service]" class="emboss" style="width: 85px; cursor: hand; cursor: pointer;" readonly="1" onclick="scwShow(this,event);" onkeydown="return false;" value="'.( $item['part_number'] ? $item['part_number'] : date('d M Y', time()+(60*60*24*30) ) ).'">';
	}
    ...
    $h .= '</td>';
	if (($item['item_id']=='0'))
        $h .= '<td class="quantity"><input style="width:50px" type="hidden" id="data[Order][BookingItem]['.$index.'][quantity]" name="data[Order][BookingItem]['.$index.'][quantity]" value="0"/>'.$item['quantity'].'</td>';
...
//		}
  ...
  function calc()
  {
	...
	function sendInvoiceToPrinter() {
		...
		$msg = file_get_contents('http://acesys.ace1.ca/index.php/orders/printView?id='.$order_id);
...
// Method renders vicidial call history into the table
	...
	function saveInvoiceTabletEmail() {
		...
		$msg = file_get_contents("http://acesys.ace1.ca/index.php/orders/invoiceTabletPrint?order_id=$order_id&type=office");
	...
	function saveInvoiceTabletItems() {
		...
		$cemail = $this->getCustomerEmailAdd($order_id);
		//echo "<pre>"; print_r($this->data['BookingItem']); echo "</pre>"; exit;
		//echo "<pre>";
		$db =& ConnectionManager::getDataSource($this->User->useDbConfig);
		...
		if(isset($_REQUEST['review'])){
			...
			echo '<BR>Redirexct =>'.$this->redirect("orders/invoiceTablet?order_id=$order_id");exit;
		}
		
		if(isset($_REQUEST['invoice'])){
			$return = $this->invoiceTabletEprint($order_id,$cemail);	
			echo '<BR>Redirexct2 =>'.$this->redirect("orders/invoiceTablet?order_id=$order_id");exit;
		}
		...
		foreach ($orders as $obj)
		{
			if (($obj['Type']['id']==9)||($obj['Type']['id']==10))
			{
				...
				if ($phone)
				{
					...
					while($row = mysql_fetch_array($result))
						$past_orders[$row['id']] = $row['id'];
                                         //$cur = $_REQUEST['order_id'];
					foreach ($past_orders as $cur)
					{
//echo '<pre>';print_r($cur);exit;
						$p_order = $this->Order->findAll(array('Order.id'=> $cur), null, "job_date DESC", null, null, 1);
						$p_order = $p_order[0];
	...
	function emailInvoiceReviewLinks($orderid,$email){
		...
		$fileUrl = "http://acecare.ca/acesys/index.php/calls/invoiceprint?order_id=".$order_id;
		set_time_limit(300);
		$subject = 'Ace Services Ltd';
    ...
    function referrals(){
		...
		$query = "select * from ace_us_referral";
