...
class OrdersController extends AppController
{
	...
	function editBooking()
	{
		...
		/*if($order_id) {
			...
			$vicidial_agent_id = $this->data['Order']['booking_telemarketer_id'];
			$vicidial_booking_date = $this->data['Order']['booking_date'];
			$vicidial_phone = $this->data['Order']['customer_phone'];
			
			if($vicidial_agent_id != '' and $vicidial_booking_date != '' and $vicidial_phone != '') {
				...
				$query = "
					...
					AND l.phone_number = $vicidial_phone
					AND DATE(rl.start_time) = '$vicidial_booking_date'
					AND rl.user = $vicidial_agent_id
					LIMIT 1
				";
				...
				$result = $db->_execute($query);
				$row = mysql_fetch_array($result);
				$recordingFile = "window.open('".$row['location']."')";
				if(!isset($recordingFile) || $row['location'] == '') $recordingFile = "alert('Recording is not available')";
			} else {
				$recordingFile = "alert('Recording is not available')";
			}			
		}*/
...
//                  echo "onclick='ShowJobChangeDetails(\"CH_DT_".$r."\",".$order_id.",".$row['id'].",\"".$row['change_date']."\",\"".$row['change_time']."\")'>";
	...
	function searchList()
	{
		...
		if ($_GET['sq_crit'] == 'phone')
		{
			...
			$sql = "SELECT User.id, User.card_number, `User`.`first_name`, User.last_name,
				  ...
				  User.lastcall_date
			FROM ace_rp_users as User 
			WHERE (phone REGEXP '".$sq_str."' 
				OR 
				cell_phone REGEXP '".$sq_str."') 
			$telem_clause
			LIMIT ".$limit;
	...
	function scheduleView()
	{
		...
		/*$query = "
			SELECT ru.route_id, ru.user_id, u.first_name name
		...
		while($row = mysql_fetch_array($result)) {
			//$routeUsers[$row['route_id']]['user_id'] = $row['user_id'];
			$routeUsers[$row['user_id']]['user_id'] = $row['user_id'];
			$routeUsers[$row['user_id']]['route_id'] = $row['route_id'];	
			$routeUsers[$row['user_id']]['name'] = $row['name'];						
		}*/
...
//	      $past_orders = $this->Order->findAll(array('Order.customer_phone'=> $phone), null, "job_date DESC", null, null, 1);
  ...
  function _itemHTML($index, $item, $actions)
  {
    ...
    if ((($item['item_id']=='1000')||($item['item_id']=='1024'))&&$actions)
...
//		}
  ...
  { 
	...
	function sendInvoiceToPrinter() {
		...
		$headers .= "Content-Type: text/html; charset=iso-8859-1\n";
		$order_id = $_GET['order_id'];
		//$msg = file_get_contents('http://69.31.184.162:81/acesys/acetest/acesys-2.0/index.php/orders/printView?id='.$order_id);
		$msg = file_get_contents('http://69.31.184.162:81/acesys/index.php/orders/printView?id='.$order_id);
...
// Method renders vicidial call history into the table
	...
	function saveInvoiceTabletEmail() {
		...
		$msg = file_get_contents("http://69.31.184.162:81/".BASE_URL."/orders/invoiceTabletPrint?order_id=$order_id&type=office");
	...
	function saveInvoiceTabletItems() {
		...
		foreach($this->data['BookingItem'] as $item_id => $item) {						
			...
			echo "$order_id, $item_id, $class, $name, $price, $quantity, $item_category_id, $price_purchase, $discount, $addition, $installed<br />";
			
			$query = "				
				INSERT INTO ace_rp_order_items (order_id, item_id, class, name, price, quantity, item_category_id, price_purchase, discount, addition, installed, print_it) 
				VALUES ($order_id, $item_id, $class, '$name', $price, $quantity, $item_category_id, $price_purchase, $discount, $addition, $installed, 'on')
			";
		...
		$last_order_id = $this->data['Order2']['id'];
		
		if(isset($last_order_id) && trim($last_order_id) != "") {
			$query = "
				DELETE FROM ace_rp_order_items
		...
		/*if($delete_this) {
			...
			if(isset($this->data['Order2']['job_date'])) {
				$job_date = $this->data['Order2']['job_date'];
				$job_time_beg = $this->data['Order2']['job_time_beg'];
				$job_time_end = $this->data['Order2']['job_time_end'];
				$job_truck = $this->data['Order2']['job_truck'];
				$job_technician1_id = $this->data['Order2']['job_technician1_id'];
				$job_technician2_id = $this->data['Order2']['job_technician2_id'];
				...
				$query = "				
						...
						SET job_date = '$job_date',
							...
							job_truck = $job_truck,
							job_technician1_id = $job_technician1_id,
							job_technician2_id = $job_technician2_id
						WHERE id = $last_order_id
					";
				$result = $db->_execute($query);
			}
			
			foreach($this->data['BookingItem2'] as $item2_id => $item2) {									
				$class = 0;
				$name = $item2['name'];
				$price = $item2['price'];
				$quantity = $item2['quantity'];
				$item_category_id = $item2['item_category_id'];
				$price_purchase = $item2['price_purchase'];
				$discount = $item2['discount'];
				$addition = $item2['addition'];
				$installed = $item2['installed'];
				
				echo "$last_order_id, $item2_id, $class, $name, $price, $quantity, $item_category_id, $price_purchase, $discount, $addition, $installed<br />";
				
				$query = "				
					INSERT INTO ace_rp_order_items (order_id, item_id, class, name, price, quantity, item_category_id, price_purchase, discount, addition, installed, print_it) 
					VALUES ($last_order_id, $item2_id, $class, '$name', $price, $quantity, $item_category_id, $price_purchase, $discount, $addition, $installed, 'on')
				";
				
				$result = $db->_execute($query);			
			}//END foreach bookingItem2		
		}//END delete_this*/		
		...
		/*if($has_booking) {
			...
			$query = "
				INSERT INTO ace_rp_orders (customer_id, booking_source_id, order_status_id, job_estimate_id, order_number, order_type_id, created_by, created_date, customer_phone)
				VALUES ($customer_id, $booking_source_id, $order_status_id, $job_estimate_id, $order_number, $booking_order_type_id, $booking_source_id, CURDATE(), $customer_phone);				
			";		
			
			$result = $db->_execute($query);
			
			$query = "
				SELECT LAST_INSERT_ID() ins;
			";
			
			$result = $db->_execute($query);
			
			while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
				$last_order_id = $row['ins'];
			}
			echo $last_order_id;		
			$this->redirect("orders/invoiceQuestions?order_id=$order_id&last_order_id=$last_order_id");
		} else {*/			
			/*if($sales_count > 0) {
				if(_needsApproval($order_id)) {
					$query = "
						INSERT INTO ace_rp_notes(message, note_type_id, order_id, user_id, urgency_id, note_date)
						SELECT CONCAT(name, ' for CAD ', (price*quantity)-discount), 
							3, $order_id, ".$this->Common->getLoggedUserID().", 2, NOW() 
						FROM ace_rp_order_items 
						WHERE order_id = $order_id			
					";
					$db->_execute($query);
	...
	function invoiceTabletFeedback() {
	 ...
	 function saveInvoiceTabletFeedback() {
		...
		/*$this->redirect("http://69.31.184.162:81/acesys/digitalsign?order_id=$order_id&email=$email");*/
		$this->redirect("orders/invoiceTabletPrint?order_id=$order_id");
	}
	...
	function invoiceTabletNotes() {
		...
		$this->set('notes', $notes);
		$this->set('needs_approval', $this->_needsApproval($order_id));
	...
	function _needsApproval($order_id) {
		...
		return ($needs_approval && $for_approval) || ($sale_approval && $sales_count);
	}	
	
	function invoiceTabletEprint() {
		$this->layout = "blank";
		$order_id = $_GET['order_id'];	
		$email = $_GET['email'];	
		$type = $_GET['type'];		
		...
		$msg = file_get_contents("http://69.31.184.162:81/".BASE_URL."/orders/invoiceTabletPrint?order_id=$order_id&type=$type");
		$res = mail($email, $subject, $msg, $headers);
	...
	function invoiceTabletNewSlot() {
		...
		$query = "
			SELECT u.first_name, u.last_name, u.address, u.city, u.postal_code,
		...
		for($i = 1; $i <= $days_ahead1; $i++) {
			...
			$query = "SELECT hs.id, hs.name, hs.hour,";				
				foreach($allslots as $id => $slot) {
					$query .= "
						(SELECT (SELECT SUBSTRING(postal_code, 1, 3) postal_code FROM ace_rp_users WHERE id = o.customer_id)
						FROM ace_rp_orders o
						WHERE job_date = DATE_ADD(CURDATE(), INTERVAL $i DAY)
